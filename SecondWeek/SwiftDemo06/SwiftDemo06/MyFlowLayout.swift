...
class MyFlowLayout: UICollectionViewFlowLayout {
    var itemW: CGFloat = 130
    var itemH: CGFloat = 200
    
    lazy var inset: CGFloat = {
        //这样设置，inset就只会被计算一次，减少了prepare的计算步骤
    ...
    override init() {
        ...
        //设置每一个元素的大小
        self.itemSize = CGSize.init(width: itemW, height: itemH)
        //设置滚动方向
    ...
    override func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        //可见矩阵
        let visiableRect = CGRect.init(x: (collectionView?.contentOffset.x)!, y: (collectionView?.contentOffset.y)!, width: (collectionView?.frame.width)!, height: (collectionView?.frame.height)!)
    ...
    override func targetContentOffset(forProposedContentOffset proposedContentOffset: CGPoint, withScrollingVelocity velocity: CGPoint) -> CGPoint {
        ...
        let lastRect = CGRect.init(x: proposedContentOffset.x, y: proposedContentOffset.y, width: (collectionView?.frame.width)!, height: (collectionView?.frame.height)!)
        //获得collectionVIew中央的X值(即显示在屏幕中央的X)
